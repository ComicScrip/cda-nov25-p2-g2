      ...
      <div className="max-w-full mx-auto mt-15 md:max-w-[600px]">
        ...
        <div className="w-[85%] py-10 bg-[#FEF9F6] rounded-4xl border-5 border-[#FFD771] mx-auto mb-10 flex items-center justify-evenly">
          ...
          <div className="text-[#1b3c79]">
            ...
            <p>{user.group?.name} ({user.group?.children?.length})</p>
          </div>
        </div>
        ...
        { user.group?.children?.length as number > 0 && 
          (
            user?.group?.children?.map(child =>
              <div  key={child.id} className="w-[85%] pt-4 pb-2 bg-[#FEF9F6] rounded-4xl border-5 border-[#FFD771] mx-auto flex flex-col items-center justify-evenly">
                <div className="overflow-hidden">
                    <img src={child.picture} alt="" className="w-65 h-[180px] object-cover shadow-gray-300 shadow-xl cursor-pointer  ease-in-out duration-300 hover:scale-110 " />
                </div> 
